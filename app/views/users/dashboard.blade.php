...
<div class="container-fluid">
    <div class="row">
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			...
			<h3 class="sub-header">({{{Auth::user()->first_name}}}) User Profile </h3>
			<table class="table table-responsive">
				...
				@foreach ($users as $user)
       			    <tr>
       			    	<td style="color:red;" class="flag"><span class="glyphicon glyphicon-flag"></span>  {{{ $users->flag_count }}}</td>
						<td>{{{ $user->first_name }}}</td>
						<td>{{{ $user->last_name }}}</td>
						<td>{{{ $user->email }}}</td>
						<td><a href="{{{ action('UserController@edit', $user->id) }}}">Edit</a>
						<td>
							{{ Form::open(array('action' => array('UserController@destroy', $foundItem->id), 'method' => 'delete')) }}
